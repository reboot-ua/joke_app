...
export const useJokes = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setLoading(true);
      const storedJokes: Joke[] = JSON.parse(
        localStorage.getItem('userJokes') || '[]',
      ).slice(0, 10);
      ...
      try {
        ...
        while (uniqueApiJokes.length < jokesNeeded) {
          ...
          const newUniqueJokes = apiJokes.filter(
            (apiJoke: Joke) =>
              !storedJokes.some((storedJoke) => storedJoke.id === apiJoke.id),
          );
          uniqueApiJokes = [...uniqueApiJokes, ...newUniqueJokes].slice(
            0,
            jokesNeeded,
          );
        }
  ...
  const handleAdd = async () => {
    try {
      ...
      if (res) {
        const isDuplicate = [...localJokes, ...jokes].some(
          (joke) => joke.id === res.id,
        );
  ...
  const handleRefresh = async (id: number) => {
    try {
      ...
      const jokeIndexInLocalStorage = storedJokes.findIndex(
        (joke: Joke) => joke.id === id,
      );
      const jokeIndexInState = combinedJokes.findIndex(
        (joke: Joke) => joke.id === id,
      );
      ...
      if (newJoke) {
        ...
        setCombinedJokes((prevState) => {
          const updatedJokes = prevState.map((joke) =>
            joke.id === id ? newJoke : joke,
          );
          return updatedJokes;
        });
      }
